...
# === Paths ===
SUBSAMPLE_CSV   = "/nvme/scratch/work/Griley/Masters/morf/subsample_photometric_ids.csv"
...
for survey, image_path in IMAGE_PATHS.items():
    ...
    for _, row in survey_df.iterrows():
        ...
        try:
            ...
            # Save FITS cutout
            fits_filename = f"{row['PROG_ID']}_{survey_id}_{FILTER}_cutout.fits"
            fits_path = os.path.join(out_dir, fits_filename)
            fits.writeto(fits_path, cutout.data, cutout.wcs.to_header(), overwrite=True)
            ...
            filename = f"{row['PROG_ID']}_{survey_id}_{FILTER}_cutout.png"            
...
# Generate morfometryka shell script
PSF_PATH = "/raid/scratch/work/Griley/GALFIND_WORK/PSFs/0.03as/F444W/PSF_Resample_03_F444W.fits"
MORFO_SCRIPT = "/nvme/scratch/work/westcottl/Codes/Morfometryka/Code/morfometryka965.py"

with open("run_morfometryka.sh", "w") as f:
    for survey, image_path in IMAGE_PATHS.items():
        survey_df = df[df["SURVEY"] == survey]
        out_dir = os.path.join(OUTPUT_DIR, survey)
        for _, row in survey_df.iterrows():
            survey_id = int(row["SURVEY_ID"])
            fits_path = os.path.join(out_dir, f"{row['PROG_ID']}_{survey_id}_{FILTER}_cutout.fits")
            f.write(f"python {MORFO_SCRIPT} {fits_path} {PSF_PATH} noshow\n")

print("Shell script saved to: run_morfometryka.sh")