...
OUTPUT_CSV = "/nvme/scratch/work/Griley/Masters/miri/F560W_fluxes.csv"
...
def measure_flux_for_object(obj_id, ra, dec, img, wcs, pixscale, ap_radius_pix, ann_inner_pix, ann_outer_pix):
    """
    Measure background-subtracted flux at RA/Dec.
    Returns flux in MicroJanskys (uJy) or None if object is invalid.
    """
    ...
    if not (0 <= x < nx and 0 <= y < ny):
        print(f"[OUT OF BOUNDS] {obj_id}: pixel=({x:.1f}, {y:.1f}) lies outside {nx}x{ny} image")
        return None
    ...
    # Background subtraction (Raw Units: MJy/sr * pixel_count)
    ann_area = annulus.area
    aper_area = aperture.area
    bkg_per_pix = ann_flux / ann_area
    bkg_total = bkg_per_pix * aper_area
    
    raw_flux = aper_flux - bkg_total

    # ------------------------------------------------
    # UNIT CONVERSION: MJy/sr -> uJy
    # ------------------------------------------------
    # 1. Calculate solid angle of one pixel in steradians
    #    (1 radian = 206265 arcsec)
    sr_per_pixel = (pixscale / 206265.0)**2

    # 2. Conversion factor: 
    #    Multiply by sr_per_pixel to remove '/sr'
    #    Multiply by 1e12 to go from MJy -> uJy
    conversion_factor = sr_per_pixel * 1e12

    flux_ujy = raw_flux * conversion_factor
...
def run_aperture_photometry(
    ...
    # ... inside run_aperture_photometry ...
    ...
    for _, row in df_radec.iterrows():
        ...
        flux = measure_flux_for_object(
            ...
            img=img,
            wcs=wcs,
            pixscale=pixscale,   # <--- ADDED THIS ARGUMENT
        ...
        rows.append({
            ...
            "ra": ra,
            "dec": dec,
            flux_column_name: flux, # This is now in uJy
        })
