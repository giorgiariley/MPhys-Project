...
def main():
    ...
    plt.ylabel(r"UV Spectral Slope ($\beta$)")
    plt.title(r"$\beta$ vs $z$ (Spectroscopy)")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("beta_vs_z_gdsgdn.png", dpi=300)

    #plot muv against redshift but make the y axis flipped so that brighter (more negative) magnitudes are higher on the plot
    plt.figure(figsize=(10, 7))
    plt.errorbar(
        df["z"], df["muv"],
        yerr=df["muv_err"],
    ...
    plt.gca().invert_yaxis()  # Flip y-axis for magnitudes
    plt.title(r"$M_{UV}$ vs $z$ (Spectroscopy)")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("muv_vs_z_gdsgdn.png", dpi=300)
