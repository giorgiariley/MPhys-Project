...
print("Single columns:", single.columns[:5].tolist())
print("Broad columns:", broad.columns[:5].tolist())
print(f"\nSingle rows: {len(single)}")
print(f"Broad rows: {len(broad)}")
print(f"\nSingle 'file' example: {single['file'].iloc[0]}")
print(f"Broad 'file' example: {broad['file'].iloc[0]}")

missing = single[~single['file'].isin(broad['file'])]
print(f"Objects in single but not broad: {len(missing)}")
print(missing[['file', 'Index']].to_string())